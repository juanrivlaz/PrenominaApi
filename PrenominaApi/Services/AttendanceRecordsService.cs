...
namespace PrenominaApi.Services
{
    public class AttendanceRecordsService : Service<AttendanceRecords>
    {
        ...
        public IEnumerable<AdditionalIncidentOutput> ExecuteProcess(GetAdditionalIncidents getAdditionalIncidents)
        {
            var queryKey = _keyRepository.GetContextEntity()
                .AsNoTracking()
                .Where(k => k.Company == getAdditionalIncidents.Company && k.TypeNom == getAdditionalIncidents.TypeNomina);

            if (getAdditionalIncidents.Tenant != "-999")
            {
                queryKey = _globalPropertyService.TypeTenant == TypeTenant.Department
                    ? queryKey.Where(k => k.Center.Trim() == getAdditionalIncidents.Tenant)
                    : queryKey.Where(k => k.Supervisor == Convert.ToDecimal(getAdditionalIncidents.Tenant));
            }
            ...
            var year = _globalPropertyService.YearOfOperation;
            var periodDates = _periodRepository.GetByFilter(
                period => period.TypePayroll == getAdditionalIncidents.TypeNomina &&
                          period.Company == getAdditionalIncidents.Company &&
                          period.NumPeriod == getAdditionalIncidents.NumPeriod &&
                          period.Year == year
            ).AsQueryable().AsNoTracking().SingleOrDefault();
            ...
            var additionalCodes = _incidentCodeRepository.GetContextEntity()
                .AsNoTracking()
            ...
            var assistanceIncidents = _assistanceIncident.GetContextEntity()
                .AsNoTracking()
                .Where(ai => employeeCodes.Contains(ai.EmployeeCode) &&
                             ai.CompanyId == getAdditionalIncidents.Company &&
                             ai.Date >= periodDates.StartDate &&
                             ai.Date <= periodDates.ClosingDate &&
                             additionalCodes.Contains(ai.IncidentCode))
                .Include(ai => ai.ItemIncidentCode)
                .Include(ai => ai.User)
                .ToList();
            ...
            var employeesDict = _employeeRepository.GetContextEntity()
                .AsNoTracking()
                .Where(item => employeeCodes.Contains(item.Codigo) &&
                               item.Company == getAdditionalIncidents.Company &&
                               item.Active == 'S')
                .ToDictionary(e => (e.Codigo, e.Company));
            ...
            return assistanceIncidents.Select(incident =>
            {
                ...
                return new AdditionalIncidentOutput
                {
                    EmployeeName = $"{employee.Name} {employee.LastName} {employee.MLastName}",
                    EmployeeCode = employee.Codigo,
                    EmployeeActivity = activityLookup.TryGetValue(employee.Codigo, out var activity) ? activity : "",
                    Date = incident.Date,
                    IncidentCode = incident.IncidentCode,
                    IncidentLabel = incident.ItemIncidentCode?.Label ?? incident.IncidentCode,
                    OperationValue = incident.MetaIncidentCode?.OperationValue,
                    WithOperation = incident.ItemIncidentCode?.WithOperation ?? false,
                    CreatedByUser = incident.User?.Name ?? "",
                    CreatedAt = incident.CreatedAt
                };
            }).Where(x => x != null).Cast<AdditionalIncidentOutput>()
            .OrderBy(x => x.EmployeeCode).ThenBy(x => x.Date);
        }
